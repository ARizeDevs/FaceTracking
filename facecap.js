...
const aspect_ratio_world = 1920 / 1080;
const render_w = 1920;
...
const controls = new OrbitControls(camera_ar, renderer_ar.domElement);
...
new GLTFLoader()
  ...
  .load('models/gltf/facecap.glb', (gltf) => {
    ...
    new GLTFLoader().load('models/gltf/mask.glb', (myModel) => {
      ...
      document.getElementById('showMask').onclick = function showSunglasses() {
    ...
    new GLTFLoader().load('models/gltf/sunglasses.glb', (myModel) => {
    ...
    new GLTFLoader().load('models/gltf/glasses.glb', (myModel) => {
...
function onResults(results) {
  ...
  if (results.multiFaceLandmarks) {
    for (const landmarks of results.multiFaceLandmarks) {
      ...
      let blendshapeScale = 1.8 * Math.sqrt(landmarkArea);
  ...
  */
      let transform = function (max_size, pt1, pt2, min_size) {
        let area = (Math.sqrt(landmarkArea) / 211).toFixed(5);
        let face_value =
          (pt1.distanceToSquared(pt2) - min_size) / (max_size * area * area);
        if (face_value > 1) {
          face_value = 1;
        }
      ...
      //smile
      max_smile = Math.max(max_smile, SmileC.distanceToSquared(SmileL));
      min_smile = Math.min(min_smile, SmileC.distanceToSquared(SmileL));
